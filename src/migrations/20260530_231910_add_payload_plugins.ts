...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`forms_rels_projects_id_idx\` ON \`forms_rels\` (\`projects_id\`);`)
  ...
  await db.run(sql`ALTER TABLE \`posts\` ADD \`tenant_id\` integer REFERENCES tenants(id);`)
  await db.run(sql`CREATE INDEX \`posts_tenant_idx\` ON \`posts\` (\`tenant_id\`);`)
  await db.run(sql`ALTER TABLE \`_posts_v\` ADD \`version_tenant_id\` integer REFERENCES tenants(id);`)
  await db.run(sql`CREATE INDEX \`_posts_v_version_version_tenant_idx\` ON \`_posts_v\` (\`version_tenant_id\`);`)
  await db.run(sql`ALTER TABLE \`projects\` ADD \`tenant_id\` integer REFERENCES tenants(id);`)
  await db.run(sql`CREATE INDEX \`projects_tenant_idx\` ON \`projects\` (\`tenant_id\`);`)
  await db.run(sql`ALTER TABLE \`_projects_v\` ADD \`version_tenant_id\` integer REFERENCES tenants(id);`)
  await db.run(sql`CREATE INDEX \`_projects_v_version_version_tenant_idx\` ON \`_projects_v\` (\`version_tenant_id\`);`)
  ...
  await db.run(sql`ALTER TABLE \`tags\` ADD \`tenant_id\` integer REFERENCES tenants(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_categories\`("id", "name", "generate_slug", "slug", "description", "updated_at", "created_at") SELECT "id", "name", "generate_slug", "slug", "description", "updated_at", "created_at" FROM \`categories\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_tags\`("id", "name", "generate_slug", "slug", "updated_at", "created_at") SELECT "id", "name", "generate_slug", "slug", "updated_at", "created_at" FROM \`tags\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "posts_id", "projects_id", "categories_id", "tags_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "posts_id", "projects_id", "categories_id", "tags_id" FROM \`payload_locked_documents_rels\`;`)
