...
/**
 * SQLite/D1 cannot reliably DROP COLUMN on `payload_locked_documents_rels` when it has
 * many foreign keys. The remove-multi-tenant migration used ALTER DROP COLUMN inside
 * runOptional(), which failed silently while `tenants` was still dropped — leaving a
 * FK to `main.tenants` and breaking any query on `payload_locked_documents`.
 */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`(
  	"id", "order", "parent_id", "path", "users_id", "media_id", "posts_id", "projects_id",
  	"categories_id", "tags_id", "forms_id", "form_submissions_id", "redirects_id",
  	"payload_mcp_api_keys_id", "search_id"
  ) SELECT
  	"id", "order", "parent_id", "path", "users_id", "media_id", "posts_id", "projects_id",
  	"categories_id", "tags_id", "forms_id", "form_submissions_id", "redirects_id",
  	"payload_mcp_api_keys_id", "search_id"
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`(
  	"id", "order", "parent_id", "path", "users_id", "tenants_id", "media_id", "posts_id", "projects_id",
  	"categories_id", "tags_id", "forms_id", "form_submissions_id", "redirects_id",
  	"payload_mcp_api_keys_id", "search_id"
  ) SELECT
  	"id", "order", "parent_id", "path", "users_id", NULL, "media_id", "posts_id", "projects_id",
  	"categories_id", "tags_id", "forms_id", "form_submissions_id", "redirects_id",
  	"payload_mcp_api_keys_id", "search_id"
  FROM \`payload_locked_documents_rels\`;`)
  await db.run(sql`DROP TABLE \`payload_locked_documents_rels\`;`)
